...
function TenantApp() {
  ...
  const loginSuccess = (user) => {
    setUserData(user);
    setIsLoggedIn(true);
    // استدعاء البيانات بدون تعليق المتصفح
    supabase.from("building_tickets").select("*").eq("building_id", user.building_id)
      .then(res => { if(res.data) setTickets(res.data); });
    supabase.from("building_votes").select("*").eq("building_id", user.building_id)
      .then(res => { if(res.data) setVotes(res.data); });
  };
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      if (data && data.length > 0) { 
        ...
        setErrorMsg("⚠️ بيانات الدخول غير صحيحة، تأكد من اختيار الدور والرمز"); 
      }
    } catch (err) { 
  ...
  const handleSendTicket = async (e) => {
    ...
    try {
      ...
      const { data } = await supabase.from("building_tickets").select("*").eq("building_id", userData.building_id);
    ...
    return (
      <div dir="rtl" style={{ minHeight: "100vh", background: "#f8fafc", padding: 16, fontFamily: "'Noto Sans Arabic', sans-serif", boxSizing:"border-box" }}>
        ...
        <div style={{ background: C.navy, color: "#fff", padding: 16, borderRadius: 16, textAlign: "center", marginBottom: 16 }}>
          ...
          <p style={{ margin: 0, fontSize: 12 }}>مبنى: {userData.building_id} | وحدة: {userData.unit} | دورك: {userData.role === 'owner' ? '👤 مالك' : userData.role === 'hoa' ? '👑 رئيس اتحاد' : '🔧 مقاول'}</p>
        </div>
