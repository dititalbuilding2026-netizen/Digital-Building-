...
function TenantApp() {
  ...
  const handleLogin = async (e) => {
    ...
    if (data && data.length > 0) { loginSuccess(data[0]); }
    else { setErrorMsg("⚠️ بيانات الدخول غير صحيحة، تأكد من اختيار الدور الصحيح والرمز"); }
    setLoading(false);
  };
        ...
        {activeMenu === "home" && (
          <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
            ...
            {userData.role === 'contractor' && (
              <div style={{ background: "#fff", padding: 16, borderRadius: 16 }}>
                <h4>🔧 تنبيه المقاول</h4>
                <p style={{ margin: 0, fontSize: 12 }}>يرجى الانتقال لقسم "الصيانة" في لوحة الإدارة لمتابعة وإصلاح بلاغات الملاك المعلقة.</p>
              </div>
            )}
          </div>
        )}
            ...
            {votes.map(v => (
              <div key={v.id} style={{ background: "#fff", padding: 16, borderRadius: 14, marginBottom: 10 }}>
                ...
                {votedList.includes(v.id) ? (
                  <div style={{ color: C.green, fontSize: 11, textAlign: "center" }}>✓ تم تسجيل تصويتك بنجاح شكراً لك</div>
                ) : (
  ...
  return (
    <div dir="rtl" style={{ minHeight: "100vh", background: "#f0f4f8", fontFamily: "'Noto Sans Arabic', sans-serif", display: "flex", flexDirection: "column", justifyContent: "center", padding: 20, boxSizing: "border-box" }}>
      <link href="https://fonts.googleapis.com/css2?family=Noto+Sans+Arabic:wght@400;700;900&display=swap" rel="stylesheet" />
      <div style={{ background: "#fff", padding: 24, borderRadius: 24, boxShadow: "0 10px 25px rgba(0,0,0,0.05)", width: "100%", maxWidth: 400, margin: "0 auto" }}>
        ...
        <form onSubmit={handleLogin}>
          ...
          <div style={{ marginBottom: 12 }}>
            <label style={{ fontSize: 12, color: C.slate }}>الدور / الصفة</label>
            <select value={credentials.role} onChange={(e)=>setCredentials({...credentials, role: e.target.value})} style={{ width: "100%", padding: 10, borderRadius: 10, border:
